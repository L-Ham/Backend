...
describe('updateEmail', () => {
  it('should update a user\'s email', async () => {
    ...
    const userMock = { 
      ...
      email: 'test@test.com', 
      save: jest.fn().mockResolvedValue({ _id: 'testUserId', email: 'new@test.com' }) 
    };
  ...
  it('should return a 400 status code if email or password is not provided', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith({ message: 'Email & Password cannot be empty' });
  });

  it('should return a 404 status code if user does not exist', async () => {
    const req = { userId: 'testUserId', body: { password: 'password123', email: 'new@test.com' } };
    const res = { status: jest.fn().mockReturnThis(), json: jest.fn() };
