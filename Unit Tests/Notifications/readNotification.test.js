...
describe('readNotification', () => {
  ...
  it('should return 400 if notification is already read', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith({ message: "Notification Already Marked as Read" });
  });

  it('should return 401 if notification was not sent to the user', async () => {
    Notification.findById.mockResolvedValue({ receiverId: 'otherUserId', isRead: false });
    User.findById.mockResolvedValue({});

    await readNotification(req, res);

    expect(res.status).toHaveBeenCalledWith(401);
    expect(res.json).toHaveBeenCalledWith({ message: "This notification wasn't sent to this user" });
  });

  it('should mark notification as read and return 200', async () => {
    const notification = { receiverId: 'userId', isRead: false, save: jest.fn() };
    ...
    expect(res.json).toHaveBeenCalledWith({ message: "Notification Marked as Read Successfully" });
  });
  ...
  it('should return 500 if there is an error', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith({ message: "Error Marking Notification as Read", error: errorMessage });
  });
});