...
describe('login', () => {
  it('should log in a user', async () => {
    ...
    expect(User.findOne).toHaveBeenCalledWith({ $or: [{ userName: req.body.userName }, { email: req.body.email }] });
    expect(bcrypt.compare).toHaveBeenCalledWith(req.body.password, 'hashedPassword');
    expect(jwt.sign).toHaveBeenCalled();
    expect(res.json).toHaveBeenCalledWith({ token: 'token', message: 'User logged in successfully' });
  });
