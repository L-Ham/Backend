const { uploadBannerImage } = require('../../controllers/userController');
...
describe('uploadBannerImage', () => {
  it('should upload a banner image', async () => {
    ...
    await uploadBannerImage(req, res, next);

    expect(User.findById).toHaveBeenCalledWith(req.userId);
    expect(UserUpload.uploadMedia).toHaveBeenCalledWith(req.files[0]);
    expect(userMock.bannerImage).toBe('imageId');
    expect(userMock.save).toHaveBeenCalled();
    expect(res.status).toHaveBeenCalledWith(200);
    expect(res.json).toHaveBeenCalledWith({ message: 'Banner image uploaded successfully', user: userMock });
  });
  ...
  it('should return a 400 status code if no file is provided', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith({ message: 'No file provided for banner image' });
  });
  ...
  it('should return a 400 status code if upload fails', async () => {
    ...
    expect(UserUpload.uploadMedia).toHaveBeenCalledWith(req.files[0]);
    expect(res.status).toHaveBeenCalledWith(400);
    expect(res.json).toHaveBeenCalledWith({ message: 'Failed to upload banner image' });
  });
  ...
  it('should return a 500 status code if an error occurs', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith({ message: 'Error uploading banner image' });
  });
});